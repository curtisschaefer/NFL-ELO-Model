...
# %%
# 1) Load Week 1 inputs (year, week, home_team, away_team, home_qb, away_qb)
elo_input_wk1 = pd.read_csv("elo_input_wk1.csv")

# Copy dicts so you don't overwrite originals
team_elo_dict = best_team_elo_sorted.copy()
qb_elo_dict   = best_qb_elo_sorted.copy()

# Unpack params
_, sigma_one, hfa, team_mean_revert, _, _ = best_params

# --- Apply team mean reversion for 2025 preseason ---
for t in team_elo_dict:
    team_elo_dict[t] -= team_mean_revert * (team_elo_dict[t] - 1500)

# Now proceed with the same predict_row function as before
def team_elo(team):
    return float(team_elo_dict.get(team, 1500.0))

def qb_elo(name):
    if pd.isna(name) or name == "":
        return 1400.0
    if name in qb_elo_dict:
        return float(qb_elo_dict[name])
    key = str(name).strip().lower()
    for k, v in qb_elo_dict.items():
        if k.strip().lower() == key:
            return float(v)
    return 1400.0

def predict_row(r):
    eh = team_elo_dict.get(r["home_team"], 1500.0)
    ev = team_elo_dict.get(r["away_team"], 1500.0)
    qh = qb_elo_dict.get(r["home_qb"], 1400.0)
    qv = qb_elo_dict.get(r["away_qb"], 1400.0)

    eh_adj = eh + (qh - 1500.0)
    ev_adj = ev + (qv - 1500.0)

    # neutral-site override: no HFA if LAC vs KC
    if (r["home_team"] == "LAC" and r["away_team"] == "KC") or \
       (r["home_team"] == "KC" and r["away_team"] == "LAC"):
        hfa_used = 0.0
    else:
        hfa_used = float(hfa)

    diff = eh_adj + hfa_used - ev_adj
    pred = abs(diff) / float(sigma_one)
    if ev_adj > eh_adj + hfa_used:
        pred *= -1.0
    return pred

elo_input_wk1["elo_spread"] = elo_input_wk1.apply(predict_row, axis=1)
